...
**Starting from an empty database:** the job tables (`jobs`, `job_events`, `bider_claims`, `bider_actors`) can be truncated at any time; settings (`bider_settings`, `scanner_control` with excluded clients, `scanner_sources`) are separate and survive. The intended flow is: paste job URLs into the dashboard Bider card → `POST /api/jobs/manual` stores them as `QUEUED` with priority ≥ 100 → the extension (Bider on, a name set) opens each one and prepares the proposal page exactly as for scanned jobs. Reload the unpacked extension **0.4.5** after `git pull`. A tab that disappears while Chrome is closed or the extension is asleep is parked as **closed**, never reopened. Adapter clicks (応募画面へ / 提案する) never count as "sent"; only your own click on a submit button does. Before 0.4.3 the extension ignored Mode (`force=true`) and reopened vanished tabs, which is why tabs appeared on their own while the dashboard said semi-auto.

**What the extension does with a URL (0.4.5, no crawling):** open the tab, click the apply control (CrowdWorks **応募画面へ**, Lancers **提案する**), and on the form that appears clear the proposal box (CrowdWorks message `<textarea>`, Lancers **提案文**) and set **完了予定日** to **one month from today** (Japan time). That is the whole list. The extension does not fetch or parse the job page, does not read title, description, client, budget, 掲載日 or 希望納期, and never sends page content anywhere; the JOB BIDER card shows only what the database already stored for that URL. It never writes into any `<input>` (契約金額（税抜） included), never clicks 新しいテンプレートを作成 (a link to `/u/message_templates/new.json`; a tab that lands there is sent back), never clicks **詳細** / 続きを読む / 詳細を書く on Lancers, and never submits. Lancers 完了予定日 handles year/month/day selects, `<input type=date>`, plain text inputs (`yyyy/mm/dd`, ISO, or 年月日) and read-only datepicker inputs (opens the calendar, moves to the month, clicks the day). Every wait is under one second. A CrowdWorks `/proposals/{id}` URL after your own submit is treated as sent. If Lancers shows ログイン / 会員登録, the extension toasts `Lancersにログインしてください` and pauses Lancers open only. The extension does **not** reload Lancers tabs on a timer.

**Window and cards:** Set **Max active** on the dashboard (for example `3`). **Fill window** opens that many queued URLs; in Mode auto a new-job alert also tops the window up. URLs pasted into the Bider inbox open as extra tabs (violet URL). Each JOB BIDER row: URL, `⏰` hourly or `💰` fixed, dates the database knows, plus **queued / ready / sent / skipped / closed**. Focus brings that job's tab and window to the front. Reload unpacked extension **0.4.5** after `git pull`.
