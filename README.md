...
The jobs table sits at the top of the page under Bider mode, max active, and the manual URL inbox. Status is per Chrome-profile name typed in the extension Options (`kenji queued`, `kenji skipped`, `kenji ready`) and resets at midnight Japan time; older Bider jobs are expired and hidden. Use **10** or **20** rows per page (default 20). Polls keep the last good rows on screen; they do not blank the table.
